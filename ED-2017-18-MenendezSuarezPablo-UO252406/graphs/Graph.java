...
public class Graph<T> {
	 ...
	 * Será el nodo más cercano al nodo más distante. Para saber el centro buscamos
	...
	/**
	 * Return the diameter of the graph, that is the longest
	...
	public double diametro() {
		double result = 0.0;
		for (int i=0;i<numNodes;i++) {
			double ex = excentricidad(nodes[i]);
			if (ex > result) {
				result = ex;
			}
		}
